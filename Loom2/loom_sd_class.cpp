...
// --- CONSTRUCTOR ---
LoomSD::LoomSD(		char* module_name, bool enable_rate_filter, uint min_filter_delay, byte chip_select, char* default_file)
...
void LoomSD::set_default_file(char* filename) 
{ 
	default_file = filename; 
}

char* LoomSD::get_default_file() 
{ 
	return default_file; 
}


void LoomSD::delete_file(char* file) 
{
	if ( !sd_found ) return;
...
void LoomSD::empty_file(char* file)
{
	if ( !sd_found ) return;
...
bool LoomSD::dump_file(char* file) 
{
	#if LOOM_DEBUG == 1
...
template <typename T>
bool LoomSD::save_elem(char *file, T data, char endchar)
{	
	if ( !sd_found ) return false;
...
void LoomSD::log_bundle(OSCBundle* bndl) 
{
...
bool LoomSD::save_bundle(OSCBundle *bndl, char * file, int timestamp)
{
	// if ( !sd_found ) return false;
...
template <typename T>
bool LoomSD::save_array(char *file, T data [], int len, char delimiter, int timestamp) 
{
	if ( !sd_found || !check_millis() ) return false;
...
void LoomSD::print_directory(File dir, int numTabs) 
{
	if ( !sd_found ) return;
