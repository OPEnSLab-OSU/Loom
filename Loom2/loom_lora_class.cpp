...
// --- CONSTRUCTOR ---
LoomLora::LoomLora( 	char* module_name,
...
bool LoomLora::receive_bundle(OSCBundle *bndl) 
{
...
bool LoomLora::send_bundle(OSCBundle *bndl, uint16_t destination) 
...
bool LoomLora::send_bundle(OSCBundle *bndl) 
{
...
void LoomLora::broadcast_bundle(OSCBundle *bndl) 
{

}


void LoomLora::set_address(uint addr)    // Need to test this
{ 
	address = addr;
	delete manager;
...
uint LoomLora::get_address() 
{ 
	return address; 
}


void LoomLora::set_friend_address(uint addr) 
{ 
	friend_address = addr; 
}


uint LoomLora::get_friend_address() 
{ 
