...
char* LoomOLED::enum_oled_version_string(OLED_Version v)
{
	switch(v) {
...
char* LoomOLED::enum_oled_format(OLED_Format f)
{
	switch(f) {
...
char* LoomOLED::enum_oled_freeze(OLED_Freeze f)
{
	switch(f) {
...
// --- CONSTRUCTOR ---
LoomOLED::LoomOLED(	char* module_name, bool enable_rate_filter, uint min_filter_delay, OLED_Version version, byte reset_pin, OLED_Format display_format, uint scroll_duration, byte freeze_pin, OLED_Freeze freeze_behavior)
...
void LoomOLED::set_display_format(OLED_Format format) 
{ 
	display_format = format; 
}


OLED_Format LoomOLED::get_display_format() 
{ 
	return display_format; 
}


void LoomOLED::set_scroll_duration(uint duration) 
{ 
	scroll_duration = duration; 
}


uint LoomOLED::get_scroll_duration() 
{ 
	return scroll_duration; 
}


void LoomOLED::set_freeze_pin(byte pin) 
{
	freeze_pin = pin;
	if (freeze_behavior != FREEZE_DISABLE) {
		pinMode(freeze_pin, INPUT_PULLUP);
...
byte LoomOLED::get_freeze_pin() 
{ 
	return freeze_pin; 
}


void LoomOLED::set_freeze_behavior(OLED_Freeze behavior) 
{ 
	freeze_behavior = behavior; 
}


OLED_Freeze LoomOLED::get_freeze_behavior() 
{ 
	return freeze_behavior; 
}


void LoomOLED::log_bundle(OSCBundle* bndl) 
{
	log_bundle(bndl, display_format);
}


void LoomOLED::log_bundle(OSCBundle* bndl, OLED_Format format) 
{
