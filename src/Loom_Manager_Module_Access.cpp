...
{
	LoomModule* tmp = find_module(LoomModule::Type::Multiplexer, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::Neopixel, idx, modules);
	return (*(Loom_Neopixel*)( (tmp) ? tmp : &global_stub ));
}

Loom_Relay& LoomManager::Relay(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::Relay, idx, modules);
	return (*(Loom_Relay*)( (tmp) ? tmp : &global_stub ));
}

Loom_Servo& LoomManager::Servo(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::Servo, idx, modules);
	return (*(Loom_Servo*)( (tmp) ? tmp : &global_stub ));
}

Loom_Stepper& LoomManager::Stepper(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::Stepper, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::LoRa, idx, modules);
	return (*(Loom_LoRa*)( (tmp) ? tmp : &global_stub ));
}

Loom_nRF& LoomManager::nRF(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::nRF, idx, modules);
	return (*(Loom_nRF*)( (tmp) ? tmp : &global_stub ));
}

Loom_Bluetooth& LoomManager::Bluetooth(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::Bluetooth, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::OLED, idx, modules);
	return (*(Loom_OLED*)( (tmp) ? tmp : &global_stub ));
}

Loom_SD& LoomManager::SDCARD(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::SDCARD, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::Analog, idx, modules);
	return (*(Loom_Analog*)( (tmp) ? tmp : &global_stub ));
}

Loom_Digital& LoomManager::Digital(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::Digital, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::Decagon5TM, idx, modules);
	return (*(Loom_Decagon5TM*)( (tmp) ? tmp : &global_stub ));
}

Loom_DecagonGS3& LoomManager::DecagonGS3(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::DecagonGS3, idx, modules);
...
{
	LoomModule* tmp = find_module(LoomModule::Type::MAX31855, idx, modules);
	return (*(Loom_MAX31855*)( (tmp) ? tmp : &global_stub ));
}

Loom_MAX31856& LoomManager::MAX31856(uint8_t idx) 
{
	LoomModule* tmp = find_module(LoomModule::Type::MAX31856, idx, modules);
