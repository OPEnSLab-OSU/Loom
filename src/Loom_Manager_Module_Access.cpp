...
{
	LoomModule* tmp = find_module(ModuleType::Multiplexer, idx, other_modules);
...
{
	LoomModule* tmp = find_module(ModuleType::Neopixel, idx, actuator_modules);
	return (*(Loom_Neopixel*)( (tmp) ? tmp : &global_stub ));
}

Loom_Relay& LoomManager::Relay(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::Relay, idx, actuator_modules);
	return (*(Loom_Relay*)( (tmp) ? tmp : &global_stub ));
}

Loom_Servo& LoomManager::Servo(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::Servo, idx, actuator_modules);
	return (*(Loom_Servo*)( (tmp) ? tmp : &global_stub ));
}

Loom_Stepper& LoomManager::Stepper(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::Stepper, idx, actuator_modules);
...
{
	LoomModule* tmp = find_module(ModuleType::LoRa, idx, comm_modules);
	return (*(Loom_LoRa*)( (tmp) ? tmp : &global_stub ));
}

Loom_nRF& LoomManager::nRF(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::nRF, idx, comm_modules);
	return (*(Loom_nRF*)( (tmp) ? tmp : &global_stub ));
}

Loom_Bluetooth& LoomManager::Bluetooth(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::Bluetooth, idx, comm_modules);
...
{
	LoomModule* tmp = find_module(ModuleType::OLED, idx, log_modules);
	return (*(Loom_OLED*)( (tmp) ? tmp : &global_stub ));
}

Loom_SD& LoomManager::SDCARD(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::SDCARD, idx, log_modules);
...
Loom_DS3231& LoomManager::DS3231(int idx) 
{
	...
	return (*(Loom_DS3231*)( (tmp && tmp->get_module_type() == ModuleType::DS3231) ? tmp : &global_stub ));
}
...
Loom_PCF8523& LoomManager::PCF8523(int idx) 
{
	...
	return (*(Loom_PCF8523*)( (tmp && tmp->get_module_type() == ModuleType::PCF8523) ? tmp : &global_stub ));
}
...
{
	LoomModule* tmp = find_module(ModuleType::Analog, idx, sensor_modules);
	return (*(Loom_Analog*)( (tmp) ? tmp : &global_stub ));
}

Loom_Digital& LoomManager::Digital(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::Digital, idx, sensor_modules);
...
{
	LoomModule* tmp = find_module(ModuleType::Decagon5TM, idx, sensor_modules);
	return (*(Loom_Decagon5TM*)( (tmp) ? tmp : &global_stub ));
}

Loom_DecagonGS3& LoomManager::DecagonGS3(int idx) 
{
	LoomModule* tmp = find_module(ModuleType::DecagonGS3, idx, sensor_modules);
...
{
	LoomModule* tmp = find_module(ModuleType::MAX31856, idx, sensor_modules);
