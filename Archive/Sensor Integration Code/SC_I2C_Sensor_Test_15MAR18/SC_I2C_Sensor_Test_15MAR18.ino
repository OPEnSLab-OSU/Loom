...
int32_t fsrLbs;       // The force converted to pound force
...
void zx_sensor_read_pos(ZX_Sensor *zx_sensor)
{
  ...
  if ( zx_sensor->positionAvailable() ) {
    ...
    if ( x_pos != ZX_ERROR ) {
      Serial.print("X: ");
      Serial.print(x_pos);
      //bndl.add(IDString "/XPosition:").add((int32_t)x_pos);
    }
    ...
    if ( z_pos != ZX_ERROR ) {
      ...
      Serial.print(z_pos);
      Serial.println("mm");
      bndl.add(IDString "/ZPosition:").add((int32_t)z_pos);
    }
   }
   else{
...
void fsr_sensor_read(){
 fsrReading = analogRead(0);  
  ...
  if (fsrVoltage == 0) {
    ...
    fsrLbs = fsrForce/4.4482216;

    bndl.add(IDString "/lbf:").add(fsrLbs);
